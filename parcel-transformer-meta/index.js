...
function getMetaTag(html, property) {
  const regex = new RegExp(`<meta[^>]*itemprop=["|']${property}["|'][^>]*>`, 'i')
...
function getMetaTagContent(metaTagHtml) {
...
module.exports = new Transformer({
  async transform({ asset }) {
    ...
    try {
      const metaImageTag = getMetaTag(html, 'image')
      console.log('🚀 ~ transform ~ ogImageTag:', ogImageTag)
